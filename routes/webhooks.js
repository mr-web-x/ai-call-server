...
// Ключевые изменения в routes/webhooks.js для поддержки streaming
...
// =====================================================
// МОДИФИЦИРОВАННЫЙ TWIML ENDPOINT
// =====================================================
...
router.post('/twiml', async (req, res) => {
  ...
  try {
    ...
    // Ищем callId из разных источников
    if (req.body?.CallSid) {
      callId = outboundManager.findCallIdByTwilioSid(req.body.CallSid);
    } else if (req.body?.callId) {
      callId = req.body.callId;
    }

    if (!callId) {
      logger.warn(`❌ Could not determine callId from request`);
      res.type('text/xml');
      res.send(outboundManager.generateErrorTwiML());
      return;
    }

    // Генерируем TwiML (теперь с поддержкой streaming)
    const twimlResponse = outboundManager.generateTwiML(callId, 'initial');
...
// =====================================================
// МОДИФИЦИРОВАННЫЙ CONTINUE ENDPOINT для streaming
// =====================================================
...
router.post('/continue/:callId', async (req, res) => {
  ...
  try {
    // Проверяем есть ли pending audio для воспроизведения
    const pendingAudio = outboundManager.getPendingAudio(callId);

    if (pendingAudio && !pendingAudio.consumed) {
      // Есть аудио для воспроизведения
      ...
      if (pendingAudio.audioUrl) {
        ...
        logger.info(`🎵 Playing audio for ${callId}: ${pendingAudio.audioUrl}`);
      } else if (pendingAudio.text) {
        twiml += `<Say voice="Polly.Tatyana" language="ru-RU">${pendingAudio.text}</Say>`;
        logger.info(
          `🗣️ Speaking text for ${callId}: ${pendingAudio.text.substring(0, 50)}...`
        );
      }
      ...
      // Нет pending audio - просто ждем
      const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Pause length="30"/>
    <Redirect>${process.env.SERVER_URL}/api/webhooks/continue/${callId}</Redirect>
</Response>`;
...
// =====================================================
// НОВЫЙ: Media Stream Status Endpoint
// =====================================================

router.post('/media-stream/status', async (req, res) => {
  const { StreamSid, Event, CallSid } = req.body;

  logger.info(`📡 Media Stream status: ${Event}`, {
    streamSid: StreamSid,
    callSid: CallSid,
  });

  if (Event === 'stream-started') {
    // Связываем stream с звонком
    const callId = outboundManager.findCallIdByTwilioSid(CallSid);
    if (callId) {
...
// =====================================================
// МОДИФИЦИРОВАННЫЙ Recording webhook (как fallback)
// =====================================================
...
router.post('/recording/:callId', async (req, res) => {
  ...
  logger.info(`🎤 Recording webhook (fallback mode) for call: ${callId}`);

  try {
    // Проверяем включен ли streaming
    const isStreamingCall =
      outboundManager.isStreamingEnabled &&
      mediaStreamManager.hasActiveStream(callId);

    if (isStreamingCall) {
      // Это не должно происходить для streaming звонков
      logger.warn(`⚠️ Recording webhook called for streaming call ${callId}`);

      // Просто продолжаем звонок
      const twiml = outboundManager.generateTwiML(callId, 'continue');
      res.type('text/xml');
      res.send(twiml);
      return;
    }

    // Обрабатываем как обычную запись (legacy mode)
    if (Digits === 'hangup') {
      await outboundManager.endCall(callId, 'client_hangup');
      res.type('text/xml');
    ...
    // Обрабатываем запись
    await outboundManager.handleRecordingReceived(
      callId,
      RecordingUrl,
      RecordingDuration,
      Digits
    ...
    // Генерируем TwiML для продолжения
    const twiml = outboundManager.generateTwiML(callId, 'continue');
    res.type('text/xml');
    res.send(twiml);
  } catch (error) {
...
// =====================================================
// НОВЫЙ: Endpoint для тестирования streaming
// =====================================================

router.get('/streaming/test/:callId', async (req, res) => {
  const { callId } = req.params;

  const streamStatus = {
    hasActiveStream: mediaStreamManager.hasActiveStream(callId),
    streamData: mediaStreamManager.getStreamInfo(callId),
    isStreamingEnabled: outboundManager.isStreamingEnabled,
    pendingAudio: outboundManager.hasPendingAudio(callId),
  };
...
// =====================================================
// Health check с информацией о streaming
// =====================================================

router.get('/health', (req, res) => {
  const activeCalls = outboundManager.getAllActiveCalls();
  const activeStreams = mediaStreamManager.getActiveStreamsCount();
  ...
  res.json({
    ...
    features: {
      streamingEnabled: outboundManager.isStreamingEnabled,
